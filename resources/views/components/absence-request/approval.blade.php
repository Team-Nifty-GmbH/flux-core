    ...
    <x-card>
        <div class="space-y-4">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <x-button
                    x-cloak
                    x-show="$wire.absenceRequestForm.status !== '{{ \FluxErp\Enums\AbsenceRequestStatusEnum::Approved->value }}'"
                    :text="__('Approve')"
                    color="emerald"
                    wire:click="approve()"
                    icon="check"
                ...
                <x-button
                    x-cloak
                    x-show="$wire.absenceRequestForm.status !== '{{ \FluxErp\Enums\AbsenceRequestStatusEnum::Rejected->value }}'"
                    :text="__('Reject')"
                    color="red"
                    wire:click="reject()"
                    icon="x-mark"
                ...
                <x-button
                    x-cloak
                    x-show="$wire.absenceRequestForm.status !== '{{ \FluxErp\Enums\AbsenceRequestStatusEnum::Revoked->value }}' && $wire.absenceRequestForm.status !== '{{ \FluxErp\Enums\AbsenceRequestStatusEnum::Pending->value }}'"
                    :text="__('Revoke')"
                    color="secondary"
                    wire:click="revoke()"
                    icon="no-symbol"
