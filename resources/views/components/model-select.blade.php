<div {{ $attributes->only(['class', 'wire:key'])->class('relative') }}
     x-data="wireui_select({
        wireModel: {{ $attributes->wire('model')->value() ?: $attributes->thatStartWith('x-model')->first() }},
    })"
     @if(! $attributes->wire('model')->value() && $attributes->thatStartWith('x-model')->first())
        x-init="$watch('wireModel', value => {{$attributes->thatStartWith('x-model')->first()}} = value)"
     @endif
     x-props="{
