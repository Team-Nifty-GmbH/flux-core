<div x-data="{
    model: $wire.entangle('{{ $attributes->wire('model')->value }}'){{ $attributes->wire('model')->hasModifier('defer') ? '.defer' : '' }},
        ...
        <div class="grid grid-cols-1 gap-3 py-2">
            <template x-for="state in availableStates">
                <div x-on:click="model = state.name" class="flex w-full cursor-pointer items-center">
                    ...
                    <div x-html="window.formatters.state(state.name, formatter[1])">
                    </div>
                </div>
            </template>
        </div>
