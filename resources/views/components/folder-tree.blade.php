<ul>
    @foreach ($tree as $treeItem)
        @if (array_key_exists("children", $treeItem))
            <li>
                <div class="block flex rounded py-1 pl-5">
                    <div
                        x-on:click="addOrRemove('{{ $treeItem["collection_name"] }}')"
                        class="grid grid-cols-2 items-center gap-x-0.5"
                    >
                        <x-icon
                            x-bind:class="{ 'rotate-90': open.indexOf('{{ $treeItem['collection_name'] }}') !== -1 }"
                            name="chevron-right"
        ...
        @else
            <li>
                <div
                    class="has-children block flex cursor-default py-1 pl-5"
                    wire:click="select({{ $treeItem["id"] }})"
                >
