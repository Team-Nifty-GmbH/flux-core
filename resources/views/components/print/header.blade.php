...
>
    ...
    <div
        ...
        x-on:mousemove.window="headerStore.isHeaderClicked ? headerStore.onMouseMoveHeader($event) : false"
        class="header-content relative h-full w-full">
    </div>
    <div
        x-on:mouseup.window="headerStore.onMouseUpResize($event)"
        x-on:mousemove.window="headerStore.isImgResizeClicked ? headerStore.onMouseMoveResize($event) : false"
    ></div>
    <template
        id="{{ $client->id }}"
