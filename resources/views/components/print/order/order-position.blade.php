...
<tbody class="bg-uneven">
    <tr>
        @if (isset($positionIdentifierWrapper))
            {{ $positionIdentifierWrapper }}
        @else
            <td class="pos py-2 pr-8 align-top">
                @if (isset($positionIdentifierContent))
                    {{ $positionIdentifierContent }}
                @else
        ...
        @else
            ...
            >
                @if (isset($positionNameContent))
                    {{ $positionNameContent }}
                @else
                    @if (isset($alternative))
                        {{ $alternative }}
                    @elseif ($position->is_alternative)
                        <x-badge
                            color="amber"
                            class="mb-2"
                            :text="__('Alternative')"
        ...
        @if (isset($positionQuantityWrapper))
            {{ $positionQuantityWrapper }}
        @else
            <td class="py-2 pr-8 text-center align-top">
                @if (isset($positionQuantityContent))
                    {{ $positionQuantityContent }}
                @elseif (! $position->is_free_text && ! $position->is_bundle_position)
        ...
        @if (isset($positionPriceWrapper))
            {{ $positionPriceWrapper }}
        @else
            <td class="py-2 text-right align-top">
                @if (isset($positionPriceContent))
                    {{ $positionPriceContent }}
                @else
