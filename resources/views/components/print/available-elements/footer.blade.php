<div x-show="printStore.editFooter" x-cloak>
    ...
    <div style="font-family: Montserrat" class="flex flex-col gap-4">
    ...
    @if ($this->client->logo_small_url)
        ...
        <div x-cloak x-show="printStore.editHeader">
            <div class="pb-4 text-lg text-gray-600">{{ __('Logo') }}</div>
            <div class="flex items-center justify-between">
                <img
                    class="h-[1.7cm]"
                    src="{{ $this->client->logo_small_url }}"
                />
                <x-toggle
    ...
    <div
        x-bind:class="{
            'pb-4':
                ...
                footerStore.visibleMedia.length > 0,
        }"
        class="flex flex-col gap-4"
    >
        <template
            x-for="(image, index) in footerStore.visibleMedia"
            :key="index"
        >
            <div class="flex items-center justify-between">
                <img
        ...
        {{-- not submited --}}
        <template
            x-for="(image, index) in footerStore.temporaryVisibleMedia"
            :key="index"
        >
            <div class="flex items-center justify-between">
                <img
    ...
    <label>
        <input
            ...
            accept="image/*"
            class="hidden"
            x-on:change="footerStore.addToTemporaryMedia($event, $refs)"
        />
        <x-button
            color="primary"
            :text="__('Add Image')"
    ...
    <div
        ...
        x-bind:class="{
            'pb-4':
                footerStore.temporarySnippetBoxes.length > 0 ||
                footerStore.visibleSnippetBoxes.length > 0,
        }"
    >
        <template
            x-for="(snippet, index) in footerStore.snippetNames"
            :key="index"
        >
            <div class="flex items-center justify-between">
                <div
