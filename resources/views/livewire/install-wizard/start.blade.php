<div>
    ...
    @if($databaseConnectionSuccessful)
        <div class="mt-5 flex flex-col gap-1.5">
            <x-button x-cloak x-show="! $wire.requestRefresh" loading color="indigo" wire:click="start(); resetProgress();" :text="__('Install')" />
            <x-button x-cloak x-show="$wire.requestRefresh" loading color="indigo" wire:click="reload().then(() => window.location.reload(true));" :text="__('Reload')" />
        </div>
    @else
        <div class="flex flex-col gap-1.5">
            ...
            <x-button color="indigo" wire:click="testDatabaseConnection()" loading :text="__('Test Database Connection')" />
        </div>
    @endif
</div>
