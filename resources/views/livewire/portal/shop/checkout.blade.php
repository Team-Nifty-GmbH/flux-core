<div class="flex flex-col gap-4 text-gray-900 dark:text-gray-50">
    ...
    <x-card :header="__('Terms And Conditions')">
        <div class="flex items-center gap-1.5">
            ...
            <div wire:click="loadTermsAndConditions().then((text) => {document.getElementById('terms-and-conditions').innerHTML = text; $modalOpen('terms-and-conditions')})" class="cursor-pointer text-primary-500 underline">
    ...
    <x-card>
        <div class="flex flex-col gap-4">
            @if(auth()->user()?->contact?->priceList?->is_net)
                <x-input :text="__('Desired delivery date')" wire:model="delivery_date" />
    ...
    <x-card :header="__('Summary')">
        ...
        <x-slot:footer>
            <x-button class="w-full" wire:click="buy()" color="indigo">{{ __('Buy now') }}</x-button>
        </x-slot:footer>
    </x-card>
