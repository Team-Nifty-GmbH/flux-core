<div x-data="{
    ...
    <div class="mx-auto max-w-screen-2xl px-4 pb-6 sm:px-6 lg:px-8">
        <div class="mx-auto md:flex md:items-center md:justify-between md:space-x-5 pb-6">
        ...
        <x-card>
            ...
            <div>
                <h3 class="text-md font-semibold mb-4">{{ __('Available Positions') }}</h3>
            ...
            <div>
                <h3 class="text-md font-semibold mb-4">{{ __('Selected Positions') }}</h3>
                <div class="space-y-2">
                                ...
                                <div class="flex justify-between items-start w-full">
                                    ...
                                    <div class="text-right ml-4">
                                        <div class="font-semibold">
                                            <span x-text="calculatePositionTotal($wire.replicateOrder.order_positions?.[{{ $index }}] || {}).toLocaleString('de-DE', {minimumFractionDigits: 2, maximumFractionDigits: 2})"></span>
                                            {{ data_get($parentOrder, 'currency.symbol') }}
                                        </div>
                                        <div class="text-sm text-gray-600 dark:text-gray-400">
                                            {{ Number::currency(data_get($position, 'is_net') ? data_get($position, 'unit_net_price') : data_get($position, 'unit_gross_price'), data_get($parentOrder, 'currency.iso', 'EUR'), 'de') }}@if(data_get($position, 'unit_abbreviation')) / {{ data_get($position, 'unit_abbreviation') }}@endif
                                        </div>
                                    </div>
                                </div>
                            </x-slot>
                ...
                >
                    <div class="flex justify-between items-center">
                        ...
                        <span class="font-bold text-lg">
                            <span x-html="window.formatters.coloredMoney(getTotalAmount(), '{{ data_get($parentOrder, 'currency.symbol') }}')"></span>
                        </span>
                    </div>
                </div>
            </div>
