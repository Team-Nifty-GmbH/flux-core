...
>
    <td class="border-b border-r bg-white px-4 py-2 pl-8 dark:border-gray-700 dark:bg-gray-900">
    ...
    >
        <div class="text-sm font-medium">
            {{ data_get($employee, 'actual_days', 0) }} / {{ data_get($employee, 'target_days', 0) }}
        </div>
    ...
    >
        <div class="text-sm font-medium">
            {{ number_format(data_get($employee, 'actual_hours', 0), 1) }} / {{ number_format(data_get($employee, 'target_hours', 0), 1) }}h
        </div>
        <div class="text-xs text-gray-500 dark:text-gray-400">
            ...
            <div class="flex flex-col items-center justify-center gap-1">
                ...
                @if ($dayData && data_get($dayData, 'is_holiday'))
                    ...
                    <div
                        ...
                        style="background-color: {{ data_get($absenceTypes, 'holiday.color', '#fee685') }}"
                    >
                        ...
                        @if ($isHalfDayHoliday)
                            ...
                            ></div>
                            <span class="absolute bottom-0 right-0.5 text-[8px] font-bold opacity-80">½</span>
                        @endif
                    </div>
                @endif
                ...
                @if ($dayData && data_get($dayData, 'actual_hours', 0) > 0)
                    <div
                        ...
                        class="flex size-10 items-center justify-center rounded border-green-500 bg-green-500 text-green-50 dark:border-transparent dark:bg-green-700 dark:bg-opacity-80 {{ data_get($dayData, 'plus_minus_overtime_hours', 0) > 0 ? 'ring-2 ring-green-400 ring-offset-1 dark:ring-green-600' : '' }} {{ data_get($dayData, 'plus_minus_overtime_hours', 0) < 0 ? 'ring-2 ring-red-400 ring-offset-1 dark:ring-red-600' : '' }}"
                        style="background-color: {{ data_get($absenceTypes, 'present.color', 'green') }}"
                    >
                ...
                @if ($dayData && data_get($dayData, 'is_daily_work_time'))
                    <div
                        ...
                        style="background-color: {{ data_get($absenceTypes, 'working.color', 'green') }}"
                    >
                ...
                @if ($dayData && ! empty(data_get($dayData, 'absence_requests')))
                    @foreach (data_get($dayData, 'absence_requests') as $absence)
                        ...
                        <div
                            ...
                            class="relative flex size-10 items-center justify-center overflow-hidden rounded font-semibold text-white"
                            style="background-color: {{ data_get($absenceTypes, data_get($absence, 'absence_type_id') . '.color', 'gray') }}"
                        >
                            <span class="truncate px-0.5 text-[10px]">
                ...
                @if ($dayData && data_get($dayData, 'is_work_day') && empty(data_get($dayData, 'absence_requests')) && data_get($dayData, 'actual_hours', 0) == 0 && (! data_get($dayData, 'is_holiday') || data_get($dayData, 'is_half_day_holiday')) && ! data_get($calDay, 'isFuture'))
                    <div
                        ...
                        style="background-color: {{ data_get($absenceTypes, 'absent.color', 'red') }}"
                    >
