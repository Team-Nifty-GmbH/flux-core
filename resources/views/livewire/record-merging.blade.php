        ...
        <div class="pl-4 pr-4">
            <x-flux::table>
                <x-slot:header>
                    <x-flux::table.row>
                        <th></th>
                        <th class="text-center" style="border-width: 1px; border-top: none; border-left: none;">
                            <span>{{ __('Id') }}</span>
                        </th>
                        <template x-for="column in $wire.columns">
                            <th class="text-center" style="border-width: 1px; border-top: none; border-right: none;">
                                <span x-text="column.label"></span>
                            </th>
                        </template>
                    </x-flux::table.row>
                </x-slot:header>
                <x-flux::table.row class="h-16" x-show="$wire.mergeRecords.main_record.id" x-cloak>
                    ...
                    <td style="border-width: 1px;">
                        <div class="flex gap-x-2">
                            ...
                            <span x-text="$wire.mergeRecords.main_record.id"></span>
                        </div>
                    </td>
                    <template x-for="column in $wire.columns">
                        <td class="text-center" style="border-width: 1px; border-right: none;">
                            <template x-if="$wire.mainRecord[column.name] === true">
                            ...
                            <template x-if="$wire.mainRecord[column.name] === false">
                            ...
                            <div x-show="typeof $wire.mainRecord[column.name] !== 'boolean'" x-cloak>
                                <span x-text="$wire.mainRecord[column.name]"></span>
                            </div>
                        </td>
                    </template>
                </x-flux::table.row>
                <template x-for="record in $wire.records">
                    <x-flux::table.row class="h-16" x-show="record.id !== $wire.mergeRecords.main_record.id" x-cloak>
                        <td></td>
                        <td style="border-width: 1px; border-left: none; border-bottom: none;">
                        ...
                        <template x-for="column in $wire.columns">
                            <td style="border-width: 1px; border-right: none; border-bottom: none;">
                                <div class="flex gap-x-2 justify-center">
                                    ...
                                    <template x-if="record[column.name] === true">
                                        <span class="bg-emerald-500 dark:bg-emerald-700 group inline-flex h-6 w-6 items-center justify-center rounded-full text-white outline-none">
                                            <svg class="h-3 w-3 shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                            </svg>
                                    ...
                                    <template x-if="record[column.name] === false">
                                         <span class="bg-red-500 dark:bg-red-700 group inline-flex h-6 w-6 items-center justify-center rounded-full text-white outline-none">
                                            <svg class="h-3 w-3 shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                                            </svg>
                                    ...
                                    <div x-show="typeof record[column.name] !== 'boolean'" x-cloak>
                                        <span x-text="record[column.name]"></span>
                                    </div>
                                </div>
                            </td>
                        </template>
