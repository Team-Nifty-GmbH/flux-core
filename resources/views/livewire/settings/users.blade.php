<div class="py-6">
    <x-modal size="6xl" id="create-user-modal" :title="__('Create User')">
        ...
        @show
        <x-slot:footer>
            <x-button color="secondary" light :text="__('Cancel')" x-on:click="$modalClose('create-user-modal')"/>
            <x-button color="indigo" :text="__('Save')" wire:click="save().then((success) => {if(success) $modalClose('create-user-modal');})"/>
        </x-slot:footer>
    </x-modal>
</div>
