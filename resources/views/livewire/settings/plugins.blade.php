...
>
    ...
    @if(resolve_static(\FluxErp\Actions\Plugins\Install::class, 'canPerformAction', [false]))
        <x-modal id="install-plugin-modal" size="7xl" :title="__('Install packages')">
            <div class="flex flex-col gap-4">
                        ...
                        <div class="flex justify-between gap-4">
                            ...
                            <div class="flex-grow">
                                <div class="flex gap-1.5">
                                    ...
                                    <x-badge color="emerald" :text="__('Flux Plugin')" x-show="plugin.is_flux_plugin" />
                                </div>
                                <div class="flex gap-1.5">
                            ...
                            <div class="flex-none flex gap-1.5">
                                <div>
                                    <x-button color="indigo" :text="__('More')" x-on:click="$wire.more(plugin.name)" />
                                    <x-button color="emerald" :text="__('Install')" x-on:click="$wire.install(plugin.name)" />
                                </div>
                            </div>
                        </div>
                    </template>
    ...
    @error('checkForUpdates')
        <div>
            <x-card class="bg-red-500 text-white gap-4 rounded-xl">
                <span>{!! $message !!}</span>
            </x-card>
        </div>
    @enderror
    ...
    <div class="flex justify-between gap-1.5 items-center">
        <x-toggle :text="__('Show only Flux Plugins')" x-model="showOnlyFluxPlugins" />
        <div class="flex gap-1.5">
            @if(resolve_static(\FluxErp\Actions\Plugins\Install::class, 'canPerformAction', [false]))
                <x-button color="indigo" :text="__('Install')" x-on:click="$modalOpen('install')" />
            @endif
            @if(resolve_static(\FluxErp\Actions\Plugins\Update::class, 'canPerformAction', [false]))
    ...
    <template x-for="(plugin, key) in installed">
        <x-card>
            <div class="flex justify-between gap-4">
                    ...
                    <div class="flex flex-col">
                        <div class="flex gap-1.5">
                            ...
                            <div x-cloak x-show="plugin.is_flux_plugin && plugin.name !== 'team-nifty-gmbh/flux-erp'">
                                <x-badge color="emerald" :text="__('Flux Plugin')" />
                            </div>
                        </div>
                ...
                <div>
                    <div class="flex-none flex gap-1.5">
                        <x-button color="indigo" :text="__('More')" wire:click="more(key)" />
                        @if(resolve_static(\FluxErp\Actions\Plugins\Install::class, 'canPerformAction', [false]))
                            <div x-cloak x-show="plugin.offer_install">
                                <x-button color="emerald" :text="__('Install')" wire:click="install(key, $promptValue('delete-data'))" />
                            </div>
                        @endif
                        @if(resolve_static(\FluxErp\Actions\Plugins\Uninstall::class, 'canPerformAction', [false]))
                        ...
                        @if(resolve_static(\FluxErp\Actions\Plugins\Update::class, 'canPerformAction', [false]))
                            <div x-cloak x-show="plugin.latest">
                                <x-button color="emerald" :text="__('Update')" wire:click="showChangeLog(key, plugin.latest)">
                                    <x-slot:label>
