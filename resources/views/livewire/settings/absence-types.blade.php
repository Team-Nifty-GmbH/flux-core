    ...
    <x-modal :id="$absenceTypeForm->modalName()" :title="__('Absence Type')">
        <div class="flex flex-col gap-4">
            <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                ...
                <div class="text-sm text-blue-800 dark:text-blue-200 space-y-3">
                    <div class="space-y-1">
                        ...
                        <p>
                            <strong>{{ __('Sick Leave') . ':' }}</strong> {{ __('Medical absence, usually with doctor\'s note') }}
                        </p>
                        <p><strong>{{ __('Vacation') . ':' }}</strong> {{ __('Uses vacation days balance') }}</p>
                        <p>
                            <strong>{{ __('Overtime Compensation') . ':' }}</strong> {{ __('Uses accumulated overtime hours') }}
                        </p>
                        <p>
                            <strong>{{ __('Normal Attendance') . ':' }}</strong> {{ __('Counts as work time (e.g., training, business school)') }}
                        </p>
                    </div>
                    ...
                    <div class="border-t border-blue-300 dark:border-blue-700 pt-3">
                        ...
                        <p>
                            <strong>{{ __(':percentage% Deduction', ['percentage' => 0]) . ':' }}</strong> {{ __('Full absence → 0h work recorded, -8h overtime') }}
                        </p>
                        <p>
                            <strong>{{ __(':percentage% Deduction', ['percentage' => 50]) . ':' }}</strong> {{ __('Half day absence → 4h work recorded, -4h overtime') }}
                        </p>
                        <p>
                            <strong>{{ __(':percentage% Deduction', ['percentage' => 100]) . ':' }}</strong> {{ __('Full day counts as worked → 8h work recorded, 0h overtime') }}
                        </p>
                    </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <x-input wire:model="absenceTypeForm.code" :label="__('Code')" required/>
            </div>
