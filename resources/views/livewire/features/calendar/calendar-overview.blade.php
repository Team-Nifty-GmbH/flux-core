            ...
            @section('calendar-edit')
                <div class="flex flex-col gap-4">
                    ...
                    <x-card :title="__('Custom Properties')">
                        <div class="flex flex-col gap-4">
                            <x-mini-button rounded class="mr-2" primary icon="plus" wire:click="addCustomProperty" />
                            @foreach($selectedCalendar['customProperties'] ?? [] as $index => $customProperty)
                                <div class="flex gap-x-4">
                                    <div class="pt-6">
                                        <x-mini-button rounded negative icon="trash" wire:click="removeCustomProperty({{ $index }})" />
                                    </div>
                                    <div class="max-w-sm">
                                        <x-select
