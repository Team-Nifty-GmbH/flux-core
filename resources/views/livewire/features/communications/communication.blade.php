        ...
        init() {
            ...
            this.$watch('modelId', () => {
                if (this.modelType && this.modelId) {
                    ...
                    Alpine.$data(
                        document
                            .getElementById('communicatable-id')
                            .querySelector('[x-data]'),
                    ).clear()
                    Alpine.$data(
                        document
                            .getElementById('communicatable-type')
                            .querySelector('[x-data]'),
                    ).clear()
                }
            })
    ...
    >
        <div class="flex flex-col gap-1.5">
            ...
            <div class="flex gap-1.5">
                <template
                    x-for="
                        (model, index) in $wire.communication.communicatables
                    "
                >
                    <x-badge flat color="indigo" cl>
                        ...
                        >
                            <button
                                type="button"
                                x-on:click="
                                    $wire.communication.communicatables.splice(
                                        index,
                                        1,
                                    )
                                "
            ...
            <div
                ...
                x-show="
                    $wire.communication.communication_type_enum ===
                        'phone-call' ||
                    $wire.communication.communication_type_enum === 'letter'
                "
            >
                <x-select.styled
                    :label="__('Address')"
            ...
            >
                        ...
                        <x-badge flat color="indigo" cl>
                            ...
                            >
                                <button
                                    type="button"
                                    x-bind:disabled="
                                        $wire.communication.id &&
                                        $wire.communication
                                            .communication_type_enum === 'mail'
                                    "
                                    x-on:click="
                                        $wire.communication.to.splice(
                                            $wire.communication.to.indexOf(to),
                                            1,
                                        )
                                    "
                                >
                ...
                <x-input
                    ...
                    x-bind:disabled="
                        $wire.communication.id &&
                        $wire.communication.communication_type_enum === 'mail'
                    "
                />
            </div>
            <div
                        ...
                        <x-badge flat color="indigo" cl>
                            ...
                            >
                                <button
                                    ...
                                    "
                                    x-on:click="
                                        $wire.communication.cc.splice(
                                            $wire.communication.cc.indexOf(to),
                                            1,
                                        )
                                    "
                                >
                        ...
                        <x-badge flat color="indigo" cl>
                            ...
                            >
                                <button
                                    ...
                                    "
                                    x-on:click="
                                        $wire.communication.bcc.splice(
                                            $wire.communication.bcc.indexOf(to),
                                            1,
                                        )
                                    "
                                >
            ...
            <div class="grow">
                <x-input
                    wire:model="communication.subject"
                    class="w-full"
                    :label="__('Subject')"
                    x-bind:disabled="
                        $wire.communication.id &&
        ...
        <x-slot:footer>
            ...
            <x-button
                ...
                x-on:click="
                    $wire.save().then((success) => {
                        if (success) $tsui.close.modal('edit-communication');
                    })
                "
                primary
                loading="save"
            ...
            >
                <x-button
                    color="indigo"
                    x-on:click="
                        $wire.send().then((success) => {
                            if (success)
                                $tsui.close.modal('edit-communication');
                        })
                    "
                    primary
                    loading="send"
    ...
    <x-modal id="create-preview" :title="__('Create Preview')">
        <div class="grid grid-cols-3 gap-1.5">
            ...
            @foreach($printLayouts as $printLayout)
                ...
                />
                <x-checkbox
                    wire:model.boolean="selectedPrintLayouts.email.{{ $printLayout }}"
                    :label="__($printLayout)"
                />
                <x-checkbox
                    wire:model.boolean="selectedPrintLayouts.download.{{ $printLayout }}"
                    :label="__($printLayout)"
                />
            @endforeach
        </div>
        <x-slot:footer>
            ...
            <x-button
                ...
                x-on:click="
                    $wire.createDocuments().then(() => {
                        $tsui.close.modal('create-preview');
                    })
                "
            />
        </x-slot:footer>
