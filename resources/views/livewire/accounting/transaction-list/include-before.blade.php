...
<x-modal id="assign-order-modal" size="7xl">
    <x-card>
        <div class="grid grid-cols-2 gap-1.5">
            ...
            <div class="overflow-auto">
                <template
                    x-for="(child, index) in $wire.transactionForm.children"
                >
                    <x-flux::list-item :item="[]">
                        <x-slot:value>
                            <span
                                x-text="
                                    child.order?.invoice_number +
                                        ' (' +
                                        window.formatters.date(child.order?.invoice_date) +
                                        ')'
                                "
                            ></span>
                        </x-slot>
                        <x-slot:sub-value>
                            <div class="flex flex-col">
                                <span
                                    x-text="child.order?.contact.invoice_address.name"
                                ></span>
                                <span
                                    x-html="
                                        '{{ __("Total") }}' +
                        ...
                        <x-slot:actions>
                            ...
                            <x-button
                                color="red"
                                icon="trash"
                                x-on:click="$wire.transactionForm.children.splice(index, 1); $wire.recalculateDifference();"
                            />
                        </x-slot>
                    </x-flux::list-item>
                </template>
    ...
    <x-slot:footer>
        <x-button
            ...
            light
            :text="__('Cancel')"
            x-on:click="$modalClose('assign-order-modal')"
        />
        <x-button
            color="indigo"
            :text="__('Save')"
            wire:click="saveAssignment().then((success) => {if(success) $modalClose('assign-order-modal');})"
        />
    </x-slot>
</x-modal>
