<div
    ...
    x-data="{
        ...
        async init() {
            ...
            const { value } = await plugins.Preferences.get({ key: 'pending_shared_files' });
            ...
            for (const meta of metas) {
                try {
                    ...
                    files.push(new File([bytes], meta.name, {
                        type: meta.mimeType || 'application/octet-stream',
                    }));
                } catch (error) {
                    console.error('[SHARE TARGET] Failed to read shared file:', meta.path, error);
                }
            }
    ...
    x-on:share-target-completed.window="cleanup().then(() => { window.location.href = $event.detail.redirect; })"
>
    <x-card>
        <x-slot:header>
        ...
        <div x-show="state === 'loading' || state === 'uploading'" x-cloak class="flex flex-col items-center gap-3 py-8">
        ...
        <div x-show="state === 'no-bridge'" x-cloak class="py-8 text-center text-sm text-gray-500 dark:text-gray-400">
            {{ __('This page is only available inside the Nuxbe mobile app.') }}
        </div>

        <div x-show="state === 'empty'" x-cloak class="py-8 text-center text-sm text-gray-500 dark:text-gray-400">
            {{ __('No shared files found. Share a file with the Nuxbe app to get started.') }}
        </div>

        <div x-show="state === 'error'" x-cloak class="py-8 text-center text-sm text-red-500">
            ...
            <div class="flex flex-col gap-2">
                @foreach ($this->files as $file)
                    <div class="flex items-center gap-3 rounded-lg border border-gray-200 p-3 dark:border-gray-700">
                        <x-icon name="document" class="h-6 w-6 shrink-0 text-gray-400" />
            ...
            <div class="flex flex-col gap-2">
                ...
                @foreach ($this->actions as $action)
                    <x-button
                        ...
                        color="primary"
                        wire:click="executeAction('{{ addslashes($action['class']) }}')"
