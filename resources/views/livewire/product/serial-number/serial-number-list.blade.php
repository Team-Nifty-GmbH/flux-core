<x-modal id="create-serial-number-modal">
    <div class="flex flex-col gap-1.5">
        ...
        />
        <x-number
            wire:model="stockPosting.address.quantity"
            :label="__('Quantity')"
        />
    </div>
    <x-slot:footer>
        <x-button
            ...
            flat
            :text="__('Cancel')"
            x-on:click="$closeModal('create-serial-number-modal')"
        />
        <x-button
            color="indigo"
            :text="__('Save')"
            x-on:click="$wire.save().then((success) => { if(success) $closeModal('create-serial-number-modal')})"
        />
    </x-slot>
</x-modal>
