<x-modal id="edit-media">
    <div class="flex flex-col gap-1.5">
        @section ('media-attributes')
            ...
            <x-input
                :label="__('File Name')"
                wire:model="mediaForm.file_name"
            />
            <x-input
                :label="__('File type')"
                disabled
                x-bind:value="$wire.mediaForm.file_name?.split('.').pop()"
            />
            <x-input
                :label="__('Disk')"
                disabled
                x-bind:value="$wire.mediaForm.disk"
            />
            <template
                x-for="(value, property) in $wire.mediaForm.custom_properties"
            >
                <div>
                    <template x-if="typeof value === 'boolean'">
                        <div class="flex gap-1.5">
                            <x-checkbox
                                ...
                                x-model.boolean="
                                    $wire.mediaForm.custom_properties[property]
                                "
                            />
                            <label
                                x-text="property"
        ...
        <x-button
            ...
            x-on:click="
                $wire.save().then((success) => {
                    if (success) $tsui.close.modal('edit-media');
                })
            "
            primary
            :text="__('Save')"
