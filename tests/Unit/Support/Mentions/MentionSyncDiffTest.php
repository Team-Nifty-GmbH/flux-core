...
test('inserts new mention rows on first sync', function (): void {
...
test('is idempotent when content is unchanged', function (): void {
    $source = CommentLikeFixture::createWithText('Hi @user:' . $this->user->getKey());
    app(MentionSync::class)->sync($source);
    app(MentionSync::class)->sync($source);
...
test('removes obsolete rows when a mention is edited away', function (): void {
...
test('returns the delta of added rows', function (): void {
