...
test('converts a record mention anchor to a token', function (): void {
    $html = 'See <a class="mention" data-type="recordMention" data-id="ticket:7" href="/tickets/7">#7</a>';

    expect(MentionHtml::toTokens($html))->toBe('See #ticket:7');
});

test('leaves non-mention anchors untouched', function (): void {
    $html = 'Visit <a href="https://example.com">site</a>';

    expect(MentionHtml::toTokens($html))->toBe($html);
});

test('leaves existing plain tokens and unrelated markup untouched', function (): void {
...
test('leaves spans whose data-id is not a key:id mention untouched', function (): void {
