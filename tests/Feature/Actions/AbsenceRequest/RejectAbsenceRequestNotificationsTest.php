...
test('rejecting notifies the requesting employees user', function (): void {
    ...
    RejectAbsenceRequest::make(['id' => $absenceRequest->getKey()])
        ->checkPermission()->validate()->execute();

    Notification::assertSentTo($employeeUser, AbsenceRequestRejectedNotification::class);
});

test('rejecting skips notification when the employee has no user', function (): void {
    $employee = CreateEmployee::make([
        'firstname' => 'Jane',
        'lastname' => 'Smith',
        'employment_date' => '2020-01-01',
