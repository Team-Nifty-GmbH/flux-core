...
test('works without an authenticated user', function (): void {
    Permission::findOrCreate('probe.guest', 'web');

    Auth::logout();

    $permissions = PermissionSet::make();

    expect($permissions->allows('probe.guest'))->toBeFalse()
        ->and($permissions->allows('never.created'))->toBeTrue();
});

test('does not count a permission the user holds on another guard', function (): void {
    Permission::findOrCreate('probe.cross-guard', 'web');
    Permission::findOrCreate('probe.cross-guard', 'api');

    $this->user->givePermissionTo(Permission::findByName('probe.cross-guard', 'web'));

    expect(PermissionSet::make(guard: 'api')->allows('probe.cross-guard'))->toBeFalse()
        ->and(PermissionSet::make(guard: 'web')->allows('probe.cross-guard'))->toBeTrue();
});

test('derives the guard from the user instead of the configured default', function (): void {
    config()->set('auth.guards.api', ['driver' => 'session', 'provider' => 'users']);
    config()->set('auth.defaults.guard', 'api');
    Permission::findOrCreate('probe.guard-derived', 'web');

    expect('web')->toBe(PermissionSet::guardFor($this->user))
        ->and(PermissionSet::make($this->user)->allows('probe.guard-derived'))->toBeFalse();
});
