<?php

use FluxErp\Models\Address;
use FluxErp\Models\Contact;

beforeEach(function (): void {
    $this->contact = Contact::factory()->create();
    $this->address = Address::factory()->create([
        'contact_id' => $this->contact->getKey(),
        'is_main_address' => true,
        'company' => 'Interaktions Test GmbH',
        'firstname' => 'Hans',
        'lastname' => 'Tester',
        'street' => 'Teststraße 1',
        'city' => 'Teststadt',
        'zip' => '12345',
    ]);
});

test('contact detail shows address data', function (): void {
    visit('/contacts/contacts/' . $this->contact->getKey())
        ->assertNoSmoke()
        ->assertSee('Interaktions Test GmbH')
        ->assertNoJavascriptErrors();
});

test('contact detail shows address street', function (): void {
    visit('/contacts/contacts/' . $this->contact->getKey())
        ->assertNoSmoke()
        ->assertSee('Teststraße 1')
        ->assertNoJavascriptErrors();
});

test('contact detail address fields are present', function (): void {
    $page = visit('/contacts/contacts/' . $this->contact->getKey())
        ->assertNoSmoke();

    $page->assertScript('!!document.querySelector("input[wire\\\\:model*=\\"address.company\\"]")');
    $page->assertNoJavascriptErrors();
});

test('contact orders tab loads', function (): void {
    $page = visit('/contacts/contacts/' . $this->contact->getKey())
        ->assertNoSmoke();

    $page->script(<<<'JS'
        () => {
            const tabs = document.querySelectorAll('[wire\\:click*="tab"]');
            for (const tab of tabs) {
                if (tab.textContent?.includes('Orders') || tab.textContent?.includes('Aufträge')) {
                    tab.click();
                    return true;
                }
            }
            return false;
        }
    JS);

    $page->assertNoJavascriptErrors();
});

test('contact accounting tab loads', function (): void {
    $page = visit('/contacts/contacts/' . $this->contact->getKey())
        ->assertNoSmoke();

    $page->script(<<<'JS'
        () => {
            const tabs = document.querySelectorAll('[wire\\:click*="tab"]');
            for (const tab of tabs) {
                if (tab.textContent?.includes('Accounting') || tab.textContent?.includes('Buchhaltung')) {
                    tab.click();
                    return true;
                }
            }
            return false;
        }
    JS);

    $page->assertNoJavascriptErrors();
});

test('contact communication tab loads', function (): void {
    $page = visit('/contacts/contacts/' . $this->contact->getKey())
        ->assertNoSmoke();

    $page->script(<<<'JS'
        () => {
            const tabs = document.querySelectorAll('[wire\\:click*="tab"]');
            for (const tab of tabs) {
                if (tab.textContent?.includes('Communication') || tab.textContent?.includes('Kommunikation')) {
                    tab.click();
                    return true;
                }
            }
            return false;
        }
    JS);

    $page->assertNoJavascriptErrors();
});

test('contact attachments tab loads', function (): void {
    $page = visit('/contacts/contacts/' . $this->contact->getKey())
        ->assertNoSmoke();

    $page->script(<<<'JS'
        () => {
            const tabs = document.querySelectorAll('[wire\\:click*="tab"]');
            for (const tab of tabs) {
                if (tab.textContent?.includes('Attachments') || tab.textContent?.includes('Anhänge')) {
                    tab.click();
                    return true;
                }
            }
            return false;
        }
    JS);

    $page->assertNoJavascriptErrors();
});

test('contact activities tab loads', function (): void {
    $page = visit('/contacts/contacts/' . $this->contact->getKey())
        ->assertNoSmoke();

    $page->script(<<<'JS'
        () => {
            const tabs = document.querySelectorAll('[wire\\:click*="tab"]');
            for (const tab of tabs) {
                if (tab.textContent?.includes('Activities') || tab.textContent?.includes('Aktivitäten')) {
                    tab.click();
                    return true;
                }
            }
            return false;
        }
    JS);

    $page->assertNoJavascriptErrors();
});

test('contact new address button exists', function (): void {
    $page = visit('/contacts/contacts/' . $this->contact->getKey())
        ->assertNoSmoke();

    $page->assertScript(<<<'JS'
        !!Array.from(document.querySelectorAll('button')).find(b =>
            b.textContent?.includes('New') || b.textContent?.includes('Neu')
        )
    JS);
});

test('address list page loads', function (): void {
    visit(route('contacts.addresses'))
        ->assertRoute('contacts.addresses')
        ->assertNoSmoke()
        ->assertPresent('[tall-datatable]')
        ->assertNoJavascriptErrors();
});

test('communications list page loads', function (): void {
    visit(route('contacts.communications'))
        ->assertRoute('contacts.communications')
        ->assertNoSmoke()
        ->assertNoJavascriptErrors();
});
